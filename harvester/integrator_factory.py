...
from integrators.polymarket import PolymarketIntegrator
...
class IntegratorFactory:
  ...
  def __init__(self) -> None:
    ...
    self._polymarket = PolymarketIntegrator()
  ...
  def polymarket_integrator(self) -> PolymarketIntegrator:
  ...
  def direct_only_sources(self) -> list[TargetSource]:
    ...
    implemented = {"polymarket"} if settings.polymarket_enabled else set()
    return [s for s in TARGET_SOURCES if s.direct_only and s.key not in implemented]
