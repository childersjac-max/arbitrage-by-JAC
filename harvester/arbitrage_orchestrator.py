...
from polymarket_fetch import fetch_polymarket_records
  ...
  *,
  ...
  try:
    ...
    if settings.polymarket_enabled:
      try:
        pm_records = await fetch_polymarket_records(factory, sport)
        records.extend(pm_records)
        logger.info("Polymarket merged %s records", len(pm_records))
