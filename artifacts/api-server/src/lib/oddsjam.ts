...
export async function getOdds(params: {
  ...
  const fixtureRes = await ojFetch<{ data: OJFixtureRaw[] }>(
    "/fixtures/active",
    fixtureParams,
  );
  ...
  return allOddsData.map((f): OJGame => {
    const fixture = fixtureMap.get(f.id);
    const homeTeam =
      f.home_team_display ?? fixture?.home_competitors[0]?.name ?? "Home";
    const awayTeam =
      f.away_team_display ?? fixture?.away_competitors[0]?.name ?? "Away";

    const bySportsbook = new Map<string, OJOddsEntry[]>();
    for (const entry of f.odds ?? []) {
      if (marketFilter && !marketFilter.some((m) => entry.market_id.includes(m)))
        continue;
    ...
    for (const [bookName, odds] of bySportsbook) {
      ...
      const bookKey = bookName
        ...
        .replace(/\s+/g, "_")
        .replace(/[^a-z0-9_]/g, "");
      bookmakers.push({ key: bookKey, title: bookName, markets: marketList });
    }
